...
if (file.exists(out_file)) {
  ...
  indie_games_genre$successful <- indie_games$successful
...
if (file.exists(out_file)) {
  ...
  indie_games_mechanic$successful <- indie_games$successful
...
```{r}
success_probability = sum(indie_games_genre$success, na.rm = TRUE)/length(indie_games_genre$success)
```


Asociation rules for genres:
```{r}
rules_success <- apriori(
  trans_indie_games_genre,
  parameter = list(
    supp   = 100/length(indie_games_genre$success),
    conf   = 0.7,
    minlen = 2
  ),
  appearance = list(
...
gm <- rules2groupedMatrix(rules_success, k = 20, lhs_label_items = 2)

# force unique group names (extra-safe: prefix group id)
labs <- paste0("G", seq_len(ncol(gm$m)), ": ", make.unique(colnames(gm$m)))
colnames(gm$m)  <- labs
colnames(gm$m2) <- labs

plot(rules_success, method = "grouped matrix", groups = gm, limit = 30)
```
```{r}
inspect(rules_success)
```
...
indie_games_genre %>%
  ...
  geom_col() +
  coord_flip() +
  labs(title = "Genre counts among successful games", x = "genre", y = "count")
```
...
plot(rules_success)
gm <- rules2groupedMatrix(rules_success, k = 10, lhs_label_items = 5)
colnames(gm$m) <- make.unique(colnames(gm$m))
plot(rules_success, method = "grouped")
```
...
indie_games_mechanic %>%
  ...
  geom_col() +
  coord_flip() +
  labs(title = "Mechanic counts among successful games", x = "mechanic", y = "count")
```
