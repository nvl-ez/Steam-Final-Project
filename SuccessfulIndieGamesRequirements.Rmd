...
make_group_flag <- function(data, values, group_name,
  ...
  # Parse list-like string: "['Action', 'Indie']"
  parse_list_row <- function(x) {
    if ( x == "[]") return(character(0))
    x <- gsub("^\\[|\\]$", "", x)
    x <- gsub("['\"]", "", x)
  ...
  # Parse dict-like string: "{'FPS': 956, 'War': 931}"
  parse_dict_row <- function(x) {
  ...
  # Precompute tokens per game once
  tokens_list <- lapply(seq_len(nrow(data)), function(i) {
    cats <- parse_list_row(data[[categories_col]][i])
    gens <- parse_list_row(data[[genres_col]][i])
    tgs  <- parse_dict_row(data[[tags_col]][i])
  ...
  flag <- vapply(
    tokens_list,
    function(tokens) any(tokens %in% values_set),
    logical(1)
  )

  data[[group_name]] <-  as.integer(flag)
  data
}
...
indie_games_genre <- indie_games_attributes %>% 
  make_group_flag(genres, action_genre, "action_genre")
