...
```{r}
tags <- get_unique_dict_keys(popularGames[popularGames$appid == 945360,]$tags)
tags
  ...
  freq_table <- as.data.frame(table(taggedGames$date_diff))
  colnames(freq_table) <- c("date_diff", "freq")
  freq_table$freq <- as.numeric(as.character(freq_table$freq))
  freq_table$date_diff <- as.numeric(as.character(freq_table$date_diff))
  freq_table
  modelAll <- lm(freq ~ date_diff, data=freq_table)
  summary(modelAll)
