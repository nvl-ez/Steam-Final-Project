...
slope_pre  <- b["t"]
slope_post <- b["t"] + b["t:post"]
delta_slope <- b["t:post"]

c(slope_pre = slope_pre, slope_post = slope_post, slope_change = delta_slope)


```
```{r}
# horizon: 20 time units after the datapoint
H <- 6

# minimum meaningful divergence (10% of pre-period SD)
pre_sd <- sd(ts_slayTheSpire$freq[ts_slayTheSpire$post == 0], na.rm = TRUE)
M <- 0.10 * pre_sd

res <- trend_change_test(
  m = m_slayTheSpire,
  H = H,
  M = M
)
...
checkIfInfluentialTag <- function(row, df, tag) {
  ...
  m <- lm(freq ~ t + post + t:post, data = ts_row)

  H <- 6
  pre_sd <- sd(ts_row$freq[ts_row$post == 0], na.rm = TRUE)
  M <- 0.10 * pre_sd

  res <- trend_change_test(m = m, H = H, M = M)

  # IMPORTANT: make this return a single TRUE/FALSE.
  # Adjust this line to match what trend_change_test returns.
  isTRUE(res) || isTRUE(res$influential) || isTRUE(res$reject)
}


checkIfInfluentialOverall <- function(i,df){
  row <- df[i, , drop = FALSE]
  tags <- get_unique_dict_keys(row$tags)
  if (length(tags) == 0) return(FALSE)
  for(tag in tags){
    if(checkIfInfluentialTag(row,df,tag)){
      return(TRUE)
    }
...
# 
# cleaned_steam$influential <- vapply(
...
build_global_timeseries <- function(games, ref_date,
  ...
  # --- compute max available months until Feb 2025 ---
  max_months_available <- as.numeric(
    difftime(max_date, ref_date, units = "days")
  ) / (365.25 / 12)

  effective_window_months <- min(window_months, floor(max_months_available))

  # --- compute time relative to launch ---
  df_raw <- games %>%
    mutate(
      t_months = as.numeric(
        difftime(release_date, ref_date, units = "days")
      ) / (365.25 / 12)
    ) %>%
    filter(t_months <= effective_window_months) %>%   # cap at Feb 2025
  ...
  # --- full bin sequence ---
  t_min <- if (bin == "month") -window_months else floor(-window_months / 12)
  t_max <- if (bin == "month") effective_window_months else floor(effective_window_months / 12)
...
```{r}
# Piecewise model (pre trend + post jump + slope change)
m_BindingOfIsaacRebirth <- lm(freq ~ t + post + t:post, data = ts_BindingOfIsaacRebirth)
summary(m_slayTheSpire)
```
