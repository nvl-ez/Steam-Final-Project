...
genre_counts %>%
  ...
  coord_flip() +
  labs(
    title = "Successful indie games — grouped genre counts",
    x = NULL,
    y = "Number of games"
  ) +
  theme_minimal()
...
```{r, echo = FALSE}
# --- MECHANIC GROUPS (project-defined) ---
...
# Generate or load Mechanics dataset (cached to disk)
...
# Filter rules whose LHS matches one of the top genre itemsets (from Section 5)
...
# Compact table of the strongest rules (ranked by lift)
...
##Pricing and owners
Finally, we explore whether price is related to estimated owners. We initially attempted to approach this with a simple scatter plot which ended up hard to interpret due to heavy overlap, price clustering, and the long-tail nature of owners.
```{r, include=FALSE}
indie_games_regression <- indie_games_data %>%
  ...
  mutate(
    ...
    num_reviews_total = as.numeric(num_reviews_total),
    recommendations   = as.numeric(recommendations),
    pct_pos_total     = as.numeric(pct_pos_total),

    owners_min = as.numeric(estimated_owners_min),
    owners_max = as.numeric(estimated_owners_max)
  ) %>%
  # Remove rows that would break log transforms (rare data anomalies)
  filter(
    !is.na(owners_min), !is.na(owners_max),
    owners_min >= 0, owners_max >= 0,
    !is.na(num_reviews_total), num_reviews_total >= 0,
    !is.na(recommendations), recommendations >= 0,
    !is.na(pct_pos_total), is.finite(pct_pos_total)
  ) %>%
  mutate(
...
price_bins <- indie_games_regression %>%
  ...
  mutate(price_bin = factor(price_bin, levels = c(
    "Free","€0–1","€1–5","€5–10","€10–20","€20–40","€40–90"
  ))) %>%
  group_by(price_bin) %>%
  summarise(
    n = n(),
    geo_mean_owners = exp(mean(log1p(owners_mid), na.rm = TRUE)) - 1,
    p10 = quantile(owners_mid, 0.10, na.rm = TRUE),
    p25 = quantile(owners_mid, 0.25, na.rm = TRUE),
    p75 = quantile(owners_mid, 0.75, na.rm = TRUE),
    p90 = quantile(owners_mid, 0.90, na.rm = TRUE),
    .groups = "drop"
  )

ggplot(price_bins, aes(x = price_bin)) +
  geom_linerange(aes(ymin = p10, ymax = p90), linewidth = 0.8) +
  geom_linerange(aes(ymin = p25, ymax = p75), linewidth = 3) +
  geom_point(aes(y = geo_mean_owners), size = 2.2) +
  geom_text(aes(y = p90, label = paste0("n=", scales::comma(n))),
            vjust = -0.7, size = 3) +
  scale_y_continuous(trans = "log1p") +
  coord_flip() +
  labs(
    title = "Owners by price tier (candlestick summary)",
    subtitle = "Point = geometric mean owners; thick bar = 25–75%; thin line = 10–90%",
    x = NULL,
    y = "Estimated owners (log1p)"
  ) +
  theme_minimal()
```
...
paid <- indie_games_regression %>%
  ...
  filter(
    if_all(
      c(log_owners, log_price, log_reviews, pct_pos_total, log_recs, dlc_count, languages_count),
      ~ is.finite(.x)
    )
  )
...
df_tiers <- S %>%
  ...
  mutate(price_tier = factor(price_tier, levels = c(
    "Free","€0–1","€1–5","€5–10","€10–20","€20–40","€40–100","€100+"
  )))

ggplot(df_tiers, aes(x = price_tier, y = owners_mid)) +
  geom_boxplot(outlier.alpha = 0.05) +
  scale_y_continuous(trans = "log1p") +
  coord_flip() +
  labs(
    title = "Owners distribution across price tiers",
    x = NULL,
    y = "Estimated owners (log1p)"
  ) +
  theme_minimal()
