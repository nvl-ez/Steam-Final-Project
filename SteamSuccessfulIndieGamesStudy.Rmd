...
```{r, echo=FALSE}
# Fit multivariate model on paid games
paid <- indie_games_regression %>%
  filter(price > 0) %>%
  mutate(
    dlc_count = as.numeric(dlc_count),
    languages_count = as.numeric(languages_count)
  ) %>%
  filter(
    if_all(
...
# VIF > 5 indicates notable multicollinearity; > 10 is often considered severe.
vif_tbl <- NULL
...
# Standardized coefficients for comparing relative impact
m_std <- lm(
...
knitr::kable(
  std_tbl,
  caption = "Standardized coefficients (relative impact comparison)"
)
```
