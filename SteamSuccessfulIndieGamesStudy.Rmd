...
## Game study: Slay the Spyre (Roguelike Deckbuilder)
...
```{r}
build_tag_timeseries <- function(games, tag_pattern, ref_date,
                                 bin = c("month", "year"),
  ...
  df <- games %>%
    ...
    mutate(
      # time difference in months relative to launch
      t_months = as.numeric(difftime(release_date, ref_date, units = "days")) / (365.25/12)
    ) %>%
    filter(abs(t_months) <= window_months) %>%
    filter(abs(t_months) > exclude_window_months)

  # binning
  df <- df %>%
    mutate(
      t_bin = if (bin == "month") floor(t_months) else floor(t_months / 12) #if we want to use months or years
    ) %>%
...
```{r}
build_tag_timeseriesV2 <- function(games, tag_pattern, ref_date,
                                 bin = c("month", "year"),
                                 exclude_window_months = 1,
                                 window_months = 60) {
  bin <- match.arg(bin)

  # 1) Filter + compute time in months
  df_raw <- games %>%
    filter(str_detect(tags, fixed(tag_pattern))) %>%
    mutate(
      t_months = as.numeric(difftime(release_date, ref_date, units = "days")) / (365.25/12)
  ...
  # 3) Create FULL sequence of bins
  t_min <- if (bin == "month") -window_months else floor(-window_months / 12)
  t_max <- if (bin == "month")  window_months else floor(window_months / 12)

  full_bins <- tibble(
    t_bin = seq(t_min, t_max, by = 1)
  )

  # 4) Join + fill missing with zero
  df_complete <- full_bins %>%
...
```{r}
ts_slayTheSpire <- build_tag_timeseries(
  games = cleaned_steam,
  tag_pattern = "Roguelike Deckbuilder",
  ref_date = ref_game_SlayTheSpire$release_date,
  bin = "month",              # months gives more detail
  exclude_window_months = 1,  # drop ±1 month around launch
  window_months = 72          # ±6 years
)

ts_slayTheSpireV2 <- build_tag_timeseriesV2(
  games = cleaned_steam,
  tag_pattern = "Roguelike Deckbuilder",
  ref_date = ref_game_SlayTheSpire$release_date,
  bin = "month",              # months gives more detail
...
m_slayTheSpireV2 <- lm(freq ~ t + post + t:post, data = ts_slayTheSpireV2)
summary(m_slayTheSpireV2)
...
ggplot(ts_slayTheSpire, aes(x = t, y = freq)) +
  ...
  labs(
    ...
    subtitle = "Tag: Roguelike Deckbuilder — binned byt = months relative to launch (t=0)",
    x = "Months relative to launch (t=0)",
    y = "Number of released games with the tag RogueLike DeckBuilding(per month bin)"
  ) +
  theme_minimal()
...
```{r}
ggplot(ts_slayTheSpireV2, aes(x = t, y = freq)) +
  geom_point(alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE, formula = y ~ x, data = subset(ts_slayTheSpireV2, t < 0)) +
  geom_smooth(method = "lm", se = TRUE, formula = y ~ x, data = subset(ts_slayTheSpireV2, t > 0)) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  labs(
    title = "Tag adoption around Slay the Spire launch",
...
build_global_timeseries <- function(games, ref_date,
  ...
  df <- games %>%
    mutate(
      t_months = as.numeric(difftime(release_date, ref_date, units = "days")) / 30.4375
    ) %>%
    filter(abs(t_months) <= window_months) %>%
    filter(abs(t_months) > exclude_window_months) %>%
    mutate(
...
```{r}
# Tag-specific (Slay the Spire effect)
ts_sts <- build_tag_timeseriesV2(
  games = cleaned_steam,
  tag_pattern = "Roguelike Deckbuilder",
  ref_date = ref_game_SlayTheSpire$release_date,
  bin = "month",
...
```{r}
m_tag <- lm(freq ~ t + post + t:post, data = ts_sts)
...
```{r}
plot_df <- bind_rows(
  ts_sts %>% mutate(type = "Roguelike Deckbuilder"),
...
```{r}
combined <- bind_rows(
  ts_sts %>% mutate(series = "tag"),
  ts_global %>% mutate(series = "global")
)
...
ts_BindingOfIsaacRebirth <- build_tag_timeseriesV2(
  games = cleaned_steam,
  tag_pattern = "Roguelike",
  ref_date = ref_game_BindingOfIsaacRebirth$release_date,
  bin = "month",              # months gives more detail
  exclude_window_months = 1,  # drop ±1 month around launch
  window_months = 120         # ±6 years
)
```
...
ggplot(ts_BindingOfIsaac, aes(x = t, y = freq)) +
  geom_point(alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE, formula = y ~ x, data = subset(ts_BindingOfIsaac, t < 0)) +
  geom_smooth(method = "lm", se = TRUE, formula = y ~ x, data = subset(ts_BindingOfIsaac, t > 0)) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  labs(
    ...
    x = "Months relative to launch (t=0)",
    y = "Number of released games with the tag RogueLike(per month)"
  ) +
  theme_minimal()
...
ggplot(ts_BindingOfIsaacRebirth, aes(x = t, y = freq)) +
  geom_point(alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE, formula = y ~ x, data = subset(ts_BindingOfIsaacRebirth, t < 0)) +
  geom_smooth(method = "lm", se = TRUE, formula = y ~ x, data = subset(ts_BindingOfIsaacRebirth, t > 0)) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  labs(
